...
class Navegador {
    async preparaNavegador() {
        ...
        if (!this.navegador) {
            let headless = false;
            this.navegador = await puppeteer_1.default.launch({ headless, ignoreHTTPSErrors: true, args });
            this.page = await this.navegador.newPage();
        }
    }
    async getData() {
        await this.page.goto("https://www.instacart.ca/store/real-canadian-superstore/storefront");
        await Helper_1.Helper.waitTillHTMLRendered(this.page);
        if (await this.page.$("#streetAddress") !== null) {
            await this.page.type("#streetAddress", "850 11 Street SW");
            await Helper_1.Helper.waitTillHTMLRendered(this.page);
            await this.sleep(2000);
            await this.page.click(".css-y9wxcl-AddressSuggestionList > li:nth-child(1) > div", { delay: 400 });
            await Helper_1.Helper.waitTillHTMLRendered(this.page);
            await this.sleep(1000);
            await this.page.click("#id-3 > div.css-1hudhoo > div.css-1rvobel > div.css-9s0vmu-UserAddressManager > form > button", { delay: 300 });
            await this.sleep(2000);
        }
        await this.page.goto("https://www.instacart.ca/store/real-canadian-superstore/collections/floral");
        await Helper_1.Helper.waitTillHTMLRendered(this.page);
        await this.autoScroll(this.page);
        this.scraper = new Scraper_1.Scraper(cheerio.load(await this.page.content()), this.navegador);
        let x = await this.scraper.getProducts();
        console.log(x);
    }
    sleep(milisegundos) {
